...
export const linkedinThumb = onRequest({ cors: true }, async (req, res) => {
  ...
  try {
    ...
    if (url.includes('/embed/')) url = url.replace('/embed/', '/');
    ...
    for (const re of regs) {
      ...
      if (m) { thumb = m[1]; break; }
    }

    if (thumb) {
      // HTML-Entities in URLs entfernen
      if (thumb.includes('&amp;')) thumb = thumb.replace(/&amp;/g, '&');
      res.json({ thumb });
    } else {
