...
import { PineconeService } from './pinecone_service';

export const avatarMemoryInsert = onRequest({
  region: 'us-central1',
  cors: true,
  invoker: 'public',
  ...
  try {
    ...
    if (mode === 'per_avatar') {
      const san = (s: string) => (s || '').toLowerCase().replace(/[^a-z0-9-]/g, '-');
    ...
    res.status(500).json({ error: error?.message || 'Memory insert failed' });
  }
});
