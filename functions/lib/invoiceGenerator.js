...
exports.ensureInvoiceFiles = exports.getInvoiceAnchorStatus = exports.ensureInvoiceForTransaction = exports.generateInvoice = void 0;
...
exports.ensureInvoiceForTransaction = functions
    .region('us-central1')
    .https.onCall(async (data, context) => {
    var _a, _b, _c, _d, _e;
    if (!context.auth)
    ...
    try {
        ...
        if (!pdfExists || !xmlExists) {
            ...
            const [pdfUrl] = await pdfFile.getSignedUrl({ action: 'read', expires: Date.now() + 30 * 24 * 3600 * 1000, responseDisposition: `attachment; filename=\"${invoiceNumber}.pdf\"` });
        ...
        const pdfSha256 = crypto_1.default.createHash('sha256').update(pdfBuf).digest('hex');
        let xmlSha256 = null;
        try {
            const [xmlBuf] = await bucket.file(xmlPath).download();
            xmlSha256 = crypto_1.default.createHash('sha256').update(xmlBuf).digest('hex');
        }
        catch (_f) {
            xmlSha256 = null;
        }
        ...
        console.warn('ensureInvoiceForTransaction soft-fail, continue with pending:', (e === null || e === void 0 ? void 0 : e.message) || e);
        const fallback = {
            anchorStatus: 'pending',
            invoiceAnchors: {
                status: 'pending',
...
exports.getInvoiceAnchorStatus = functions
    .region('us-central1')
    .https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError('unauthenticated', 'Nicht angemeldet');
    ...
    if (!txSnap.exists)
        throw new functions.https.HttpsError('not-found', 'Transaktion nicht gefunden');
    const tx = txSnap.data() || {};
    const anchorStatus = tx.anchorStatus || 'not_anchored';
    const ia = tx.invoiceAnchors || {};
    return {
        status: anchorStatus,
