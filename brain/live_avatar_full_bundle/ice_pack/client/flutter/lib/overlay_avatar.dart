...
class VisemeMixer with ChangeNotifier {
  ...
  void updateWeights(Map<String, double> w) {
    final items = w.entries.toList()..sort((x,y)=>y.value.compareTo(x.value));
...
class AvatarOverlay extends StatelessWidget {
  ...
  const AvatarOverlay({super.key, required this.atlas, required this.mask, required this.cells, required this.roi, required this.mixer});
...
class _Painter extends CustomPainter {
  final ui.Image atlas, mask; final Map<String, Rect> cells; final Rect roi; final VisemeMixer mixer;
  _Painter(this.atlas,this.mask,this.cells,this.roi,this.mixer):super(repaint:mixer);
  ...
  void paint(Canvas c, Size s) {
    ...
    c.drawImageRect(atlas, srcA, roi, p..color = const Color(0xffffffff).withOpacity(1.0 - k));
    c.drawImageRect(atlas, srcB, roi, p..color = const Color(0xffffffff).withOpacity(k));
    c.drawImageRect(mask, Rect.fromLTWH(0,0,mask.width.toDouble(),mask.height.toDouble()), roi,
...
void onVisemeMessage(String jsonStr, VisemeMixer mixer) {
  ...
  final weights = (m["weights"] as Map).map((k,v)=>MapEntry(k as String, (v as num).toDouble()));
  mixer.updateWeights(weights);
}

void onProsodyMessage(String jsonStr, ProsodyState state) {
  final m = json.decode(jsonStr) as Map<String,dynamic>;
  state.pitch = (m["pitch"] as num).toDouble();
  state.energy = (m["energy"] as num).toDouble();
