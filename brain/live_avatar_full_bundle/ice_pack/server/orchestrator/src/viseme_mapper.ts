import type { TimestampEvent, VisemeEvent } from "./types";
...
export class VisemeMapper {
  ...
  consumeTimestamp(ts: TimestampEvent): VisemeEvent | null {
    const t = ts.t_ms;
    this.lastT = t;
    ...
    for (const k in weights) weights[k] = weights[k] / s;
    ...
    const zeta = 1.0, wn = 14.0; // tune as needed
